...
import { gameLaunchErrorMessage } from "./game-launch-error";
...
ipcMain.handle("game:play", async () => { const config = await readConfig(); if (!config.gameDirectory || !(await isClientInstalled(config.gameDirectory))) throw new Error("Client não encontrado. Instale-o novamente."); try { await proxy.start(); game = spawn(clientPath(config.gameDirectory), [], { cwd: config.gameDirectory, windowsHide: false }); game.once("error", async (error) => { await proxy.stop(); console.error("Falha ao iniciar o jogo:", error); window.webContents.send("game-error", { message: gameLaunchErrorMessage(error) }); game = undefined; }); game.once("exit", async () => { await proxy.stop(); game = undefined; window.webContents.send("game-ended"); }); return true; } catch (error) { await proxy.stop(); const e = error as NodeJS.ErrnoException; if (e.code === "EADDRINUSE") throw new Error("A porta 8281 já está em uso, provavelmente por uma regra portproxy. Execute como administrador: netsh interface portproxy delete v4tov4 listenaddress=127.0.0.1 listenport=8281"); throw error; } });
