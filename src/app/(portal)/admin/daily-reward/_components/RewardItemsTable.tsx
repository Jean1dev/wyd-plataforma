...
function Row({ item }: { item: DailyRewardItem }) {
  ...
  async function toggle() {
    ...
    setEnabled(next);
    try {
      await setRewardItemEnabled(item.id, next);
      router.refresh();
    } catch (err) {
      setEnabled(!next);
  ...
  if (editing) {
    return (
      <tr>
        <td colSpan={6} style={{ ...cell, padding: 16 }}>
          <RewardItemForm item={item} onDone={() => setEditing(false)} />
        </td>
      </tr>
    );
  }
...
export function RewardItemsTable({ items }: { items: DailyRewardItem[] }) {
  if (items.length === 0) {
    return (
