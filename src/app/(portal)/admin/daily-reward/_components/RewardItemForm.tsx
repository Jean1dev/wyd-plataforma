...
import { createRewardItem, errorMessage, updateRewardItem } from "./api";

const emptyItem: DailyRewardItemPayload = {
  item_index: 0,
  eff1: 0,
  effv1: 0,
  eff2: 0,
...
export function RewardItemForm({
  item,
  onDone,
}: {
  ...
  const [form, setForm] = useState<DailyRewardItemPayload>(item ?? emptyItem);
  const [busy, setBusy] = useState(false);
  const [msg, setMsg] = useState<{ kind: "ok" | "error"; text: string } | null>(null);

  function set<K extends keyof DailyRewardItemPayload>(key: K, value: DailyRewardItemPayload[K]) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      if (editing && item) {
        await updateRewardItem(item.id, form);
  ...
  return (
    <form onSubmit={submit} style={{ display: "grid", gap: 14 }}>
      <Input label="Título" name="title" value={form.title} onChange={(e) => set("title", e.target.value)} required />
